...
# === Optional: Start hot reload (watch mode) ===
echo ""
read -p "Do you want to start in watch (hot reload) mode now? [y/N]: " answer
if [[ "$answer" == "y" || "$answer" == "Y" ]]; then
    print_green "Starting in watch mode using utils/watch.py..."
    python utils/watch.py
else
    print_blue "You can later start watch mode manually with:"
    echo "python utils/watch.py"
fi
